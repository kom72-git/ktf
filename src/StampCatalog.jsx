              ...
              <div key={group}>
                ...
                <div className="variants">
                  ...
                  {defs.filter(def => def.variantaVady && def.variantaVady.length === 1).map((def, i) => {
                    ...
                    return (
                      <div key={def.idVady || `main-${i}`} className="variant" style={{ position: 'relative' }}>
                            ...
                            <div style={{ marginTop: '4px', display: 'flex', gap: '4px' }}>
                              <button
                                onClick={(e) => {
                                  ...
                                  console.log('=== SAVE ALL VARIANT FIELDS ===');
                                  console.log('def object:', def);
                                  console.log('def._id:', def._id);
                                  console.log('varianta:', variantInput?.value);
                                  console.log('umisteni:', umisteniInput?.value);
                                  console.log('popis:', popisTextarea?.value);
                  ...
                  {(() => {
                    ...
                    return defs.filter(def => {
                      ...
                      const flatIndex = itemDefects.findIndex(d => d === def);
                      return (
                        <div key={def.idVady || `sub-${i}`} className="variant" style={{ position: 'relative' }}>
                          <div className="variant-popis">
                            {isEditingAll ? (
                              <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '8px' }}>
                                <input
                          ...
                          {/* Editace URL obrázku vady pro podvarianty */}
                          {isEditingAll && (
                            <div style={{ marginTop: '8px' }}>
                              <div style={{ display: 'flex', gap: '5px', alignItems: 'center' }}>
                                <input
                          ...
                          <div className="variant-label">Obr. {getGlobalImageNumber(def)}</div>
                          {/* Editace nebo zobrazení popisu vady pro podvarianty */}
                          {isEditingAll ? (
                            <div style={{ marginTop: '8px' }}>
                              <textarea
                                defaultValue={def.popisVady || ''}
                                onKeyPress={(e) => {
                                  if (e.key === 'Enter' && e.ctrlKey) {
                                    saveDefectEdit(def._id, { ...def, popisVady: e.target.value });
                                  }
                                }}
                                style={{
                                  width: '100%',
                              ...
                              <div style={{ marginTop: '4px', display: 'flex', gap: '4px' }}>
                                <button
                                  onClick={(e) => {
                                    ...
                                    console.log('=== SAVE ALL SUBVARIANT FIELDS ===');
                                    console.log('def object:', def);
                                    console.log('def._id:', def._id);
                                    console.log('varianta:', variantInput?.value);
                                    console.log('umisteni:', umisteniInput?.value);
